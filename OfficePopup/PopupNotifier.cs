...
namespace DotNetControlsEx.OfficePopup
{
    ...
    public class PopupNotifier : Component
    {
        ...
        protected bool waitingForUserActivity = true;
        ...
        [Description("Whether or not to wait for user activity before removing notification")]
        public bool WaitForUserActivity { get; set; }
            ...
            if (!disposed)
            {
                if (!frmPopup.Visible)
                {
                    if (this.WaitForUserActivity)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine("Add Mouse Hook.");
                        ...
                        System.Diagnostics.Debug.WriteLine("Add Keyboard Hook.");
                    }
        ...
        private void tmAnimation_Tick(object sender, EventArgs e)
        {
            ...
            if (elapsed > AnimationDuration)
            {
                ...
                if (isAppearing)
                {
                    ...
                    if (!mouseIsOn)
                    {
                        if ((WaitForUserActivity && waitingForUserActivity)) return; //still waiting for user activity
        ...
        protected int mouseHookProc(int nCode, int wParam, IntPtr lParam)
        {
            ...
            {
                if (waitingForUserActivity && WaitForUserActivity)
                {
                    waitingForUserActivity = false;
        ...
        {
            if (nCode == NativeMethods.HC_ACTION && waitingForUserActivity && WaitForUserActivity)
            {
                waitingForUserActivity = false;
        ...
        private void RemoveHooks()
        {
            if (hMouseHook != 0)
            {
                ...
                hMouseHook = 0;
                System.Diagnostics.Debug.WriteLine("Remove Mouse Hook.");
            }
            if (hKeyboardHook != 0)
            {
                NativeMethods.UnhookWindowsHookEx(hKeyboardHook);
                hKeyboardHook = 0;
                System.Diagnostics.Debug.WriteLine("Remove Keyboard Hook.");
            }
        }
